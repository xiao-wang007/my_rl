...
# Suppress gym deprecation warnings from third-party packages
warnings.filterwarnings("ignore", message=".*Gym has been unmaintained.*")
# Suppress PPO GPU warning (we know MLP policies don't benefit from GPU)
...
def main():
    ...
    zip = f"data/panda_reach_ppo_{config['observations']}_net{config['net_arch'][0]}_vScale{config['v_max_scale']}_smoothness_reward_improvedController.zip"
    ...
    model.learn(
        total_timesteps=6e6 if not args.test else 4,
        callback=callbacks,
    )
    model.save(zip)
